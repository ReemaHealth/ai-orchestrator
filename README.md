...
Default: `AGENT_ENABLED=false` uses skeleton agent chunks locally. Set `AGENT_ENABLED=true` and Vertex env vars to call Reasoning Engine (requires ADC — see [docs/testing.md](docs/testing.md)). Configure `GOOGLE_OAUTH_*` for server-side user OAuth (workspace datastore access).
...
| GET | `/api/v1/oauth/google/start` | Firebase JWT |
| GET | `/api/v1/oauth/google/callback` | Signed OAuth state |
| DELETE | `/api/v1/oauth/google/revoke` | Firebase JWT |
