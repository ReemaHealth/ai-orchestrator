...
| 400 | missing `prompt` |
| 428 | Google OAuth consent required |
...
GOOGLE_OAUTH_REDIRECT_URI=http://localhost:8080/api/v1/oauth/google/callback \
GOOGLE_OAUTH_SUCCESS_REDIRECT=http://localhost:5670 \
go run .
```

**One-time Google OAuth consent** (browser, while logged into care portal / with Firebase JWT):

1. Open `GET http://localhost:8080/api/v1/oauth/google/start` with header `Authorization: Bearer <firebase-id-token>` (or link from care portal).
2. Complete Google consent; callback stores refresh token.
3. `POST /api/v1/prompt` with Firebase JWT only — no `X-GCP-Access-Token` needed.

Example prompt (replace Firebase token):
...
If consent is missing, response is **428**:

```json
{"error":"google_oauth_required","authorizePath":"/api/v1/oauth/google/start","message":"..."}
```

**Optional curl override** (skip server-side OAuth):

```bash
curl -N -X POST http://localhost:8080/api/v1/prompt \
  -H "Authorization: Bearer <firebase-id-token>" \
  -H "X-GCP-Access-Token: $(gcloud auth print-access-token)" \
  -H "Content-Type: application/json" \
  -d '{"prompt":"..."}'
```

Or set `USER_OAUTH_MODE=client_only` and omit `GOOGLE_OAUTH_*` to require client-supplied tokens only.

Cloud Run: grant the orchestrator service account `roles/aiplatform.user` (and reasoning engine invoke per your org setup). Store `GOOGLE_OAUTH_CLIENT_SECRET` in Secret Manager.
